...
VkSemaphore *create_semaphores(VkDevice device, uint32_t count) {
    ...
    for (uint32_t i = 0; i < count; i++) { vkCreateSemaphore(device, &smCInfo, VK_NULL_HANDLE, semaphores + i); }
    return semaphores;
}
...
VkFence *create_fences(VkDevice device, uint32_t count) {
    VkFenceCreateInfo fcCInfo = {
        .sType = VK_STRUCTURE_TYPE_FENCE_CREATE_INFO, .pNext = VK_NULL_HANDLE, .flags = VK_FENCE_CREATE_SIGNALED_BIT};
    ARR_ALLOC(VkFence, fences, count);
    for (uint32_t i = 0; i < count; i++) { vkCreateFence(device, &fcCInfo, VK_NULL_HANDLE, fences + i); }
    return fences;
}
